...
public class DBReceta {
    ...
    public boolean modificarReceta(String nombrereceta, String descripcion, ArrayList<String> ingredientes, ArrayList<String> unidadmetrica, ArrayList<Integer> cantidad, String seleccionado) {
        try {
            ...
            consulta = "UPDATE receta SET nombre_receta = ?, descripcion = ? WHERE nombre_receta = ?";
            preparedStatement = connection.prepareStatement(consulta);
            preparedStatement.setString(1, nombrereceta);
            preparedStatement.setString(2, descripcion);
    ...
    public String[] cargarStringRecetas(String seleccionado) {
        ...
        try {
            ...
            consulta = "SELECT * FROM receta WHERE nombre_receta = ?";
            preparedStatement = connection.prepareStatement(consulta);
            preparedStatement.setString(1, seleccionado);
            ResultSet resultSet = preparedStatement.executeQuery();

            while (resultSet.next()) {
                retorno[0] = resultSet.getString("nombre_receta");
                retorno[1] = resultSet.getString("descripcion");
            }
            return retorno;
