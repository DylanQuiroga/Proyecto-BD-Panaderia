...
bars = plt.bar(df['Dia'], df['Cantidad'])
plt.ylabel('Cantidad')
plt.title('Cantidad generada por dia')
...
for i, bar in enumerate(bars):
    yval = bar.get_height()
    plt.text(bar.get_x() + bar.get_width()/2, yval/2, df['Dia'][i], ha='center', va='bottom', fontsize=14, rotation=90)
